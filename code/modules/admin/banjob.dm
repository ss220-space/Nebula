...
/proc/jobban_loadbanfile()
	if(config.ban_legacy_system)
		...
		//Job permabans
		var/DBQuery/query = dbcon.NewQuery("SELECT `ckey`, `job` FROM [sqldbutil].ban WHERE `bantype` = 'JOB_PERMABAN' AND isnull(`unbanned`)")
		query.Execute()
		...
		//Job tempbans
		var/DBQuery/query1 = dbcon.NewQuery("SELECT `ckey`, `job` FROM [sqldbutil].ban WHERE `bantype` = 'JOB_TEMPBAN' AND isnull(`unbanned`) AND `expiration_time` > Now()")
		query1.Execute()
